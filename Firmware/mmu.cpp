...
//	printf_P(PSTR("MMU loop, state=%d\n"), mmu_state);
	...
	case S::GetFinda: //response to command P0
        if (mmu_idl_sens)
        {
            if (PIN_GET(IR_SENSOR_PIN) == 1 && mmu_loading_flag) //was 0
            {
#ifdef MMU_DEBUG
                printf_P(PSTR("MMU <= 'A'\n"));
#endif //MMU_DEBUG  
	...
	while (!mmu_ready)
	{
		...
		switch (move) {
			case MMU_LOAD_MOVE:
				...
				if (PIN_GET(IR_SENSOR_PIN) == 1) move = MMU_NO_MOVE; //was 0
				break;
			case MMU_UNLOAD_MOVE:
				if (PIN_GET(IR_SENSOR_PIN) == 1) //filament is still detected by idler sensor, printer helps with unlading  //was 0
				{
				    if (can_extrude())
				    {
			...
			case MMU_TCODE_MOVE: //first do unload and then continue with infinite loading movements
				if (PIN_GET(IR_SENSOR_PIN) == 1) //filament detected by idler sensor, we must unload first //was 0
				{
                    if (can_extrude())
                    {
...
static bool can_load()
{
    ...
    for(uint_least8_t i = 0; i < steps; ++i)
    {
        ...
        plan_buffer_line_curposXYZE(MMU_LOAD_FEEDRATE);
        st_synchronize();
        if(1 == PIN_GET(IR_SENSOR_PIN)) //was 0
        {
...
static bool load_more()
{
    ...
    {
        if (PIN_GET(IR_SENSOR_PIN) == 1) return true; //was 0
